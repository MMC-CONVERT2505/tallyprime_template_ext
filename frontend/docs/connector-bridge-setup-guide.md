...
> ```
>
> Same connection id, same `defaultCompany`, brand-new secret — the old token stops working immediately.
...
| `fetch-master` resolved a company to a connector you didn't expect | Legacy duplicate rows from before this session's upsert fix (`POST /connections`/pairing are idempotent per company now, so this shouldn't recur) | `ExtractionsService.resolveConnectionByCompany` auto-picks whichever candidate is currently online, tie-broken by most-recent `lastSeenAt` — it no longer errors on ambiguity. `GET /connections` to see every candidate; `POST /connections/:id/revoke` any stale one you don't want considered. |

...
|---|---|---|
| Configure + trigger Tally export | User navigates Tally's export UI, sets 5+ options correctly, saves XML by hand | The extraction services ([src/tally/extraction](../src/tally/extraction)) talk to Tally's HTTP/XML server directly — no export dialog, no file, no manual settings. We control exactly what's requested. |

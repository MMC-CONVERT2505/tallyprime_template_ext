...
Scope is the authoritative 33-function list in [tally-zoho-function-mapping.md](tally-zoho-function-mapping.md) (from the project's own Tally↔Zoho mapping reference, not inferred from the Zoho migration tool's config). Modeled end-to-end today: Companies (#1), Ledgers (#2/#7/#8), Groups (#4), Stock Items (#9) — including period-scoped (`fromDate`/`toDate`) Opening/Closing balances — and Vouchers (#14-24/#30-33, filterable by voucher type). Masters came first (they're what let transactions be mapped correctly at all); the remaining masters (Stock Groups, Stock Categories, Godowns, Cost Centres, Tax, Currencies) and the two-Zoho-targets-from-one-Tally-voucher-type cases (#18/#22, #19/#21) are still open — mechanical per-entity, same builder/parser/interface pattern as the existing implementations, added one at a time as actually needed rather than speculatively.

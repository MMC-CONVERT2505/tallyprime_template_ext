...
describe('ExtractionsService', () => {
  ...
  function makeDeps(overrides: {
    ...
    const tunnel = {
      listConnectedAgents: overrides.tunnel?.listConnectedAgents ?? jest.fn().mockReturnValue(['conn-1']),
    };
    ...
    const service = new ExtractionsService(prisma as any, queue as any, tunnel as any, redis as any, excel as any);
  ...
  describe('create', () => {
    ...
    it("falls back to the connection's defaultCompany when payload.company is omitted, AND actually threads it through to the dispatched job (not just the DB record)", async () => {
      ...
      await service.create('org-1', 'user@example.com', { connectionId: 'conn-1', type: 'LEDGERS' as any });
    ...
    it('rejects a connection that does not belong to the caller\'s org (or does not exist)', async () => {
      ...
      await expect(
        service.create('org-1', 'user@example.com', { connectionId: 'conn-1', type: 'LEDGERS' as any }),
    ...
    it('rejects immediately when the connector is not currently online, rather than queueing a doomed job', async () => {
      const { service, queue } = makeDeps({ tunnel: { listConnectedAgents: jest.fn().mockReturnValue([]) } });
  ...
  describe('fetchMaster', () => {
    function makeFetchMasterDeps(overrides: { findMany?: jest.Mock; tunnel?: jest.Mock } = {}) {
      ...
      const queue = { add: jest.fn().mockResolvedValue({}) };
      const tunnel = { listConnectedAgents: overrides.tunnel ?? jest.fn().mockReturnValue(['conn-1']) };
    ...
    it('404s when no connector is paired with the requested company', async () => {
      ...
      await expect(
        service.fetchMaster('org-1', 'user@example.com', { companyName: 'Nope Ltd', masterType: 'LEDGERS' as any }),
      ).rejects.toThrow(NotFoundException);
      expect(queue.add).not.toHaveBeenCalled();
    });

    it('rejects ambiguously when multiple connectors share the same paired company, listing the candidate ids', async () => {
      const { service } = makeFetchMasterDeps({
        findMany: jest.fn().mockResolvedValue([CONNECTION, { ...CONNECTION, id: 'conn-2' }]),
      });

      await expect(
        service.fetchMaster('org-1', 'user@example.com', { companyName: 'ABC Ltd', masterType: 'LEDGERS' as any }),
      ).rejects.toThrow('specify connectionId');
      await expect(
        service.fetchMaster('org-1', 'user@example.com', { companyName: 'ABC Ltd', masterType: 'LEDGERS' as any }),
      ).rejects.toThrow(/conn-1, conn-2/);
    });
  ...
  describe('getStatus', () => {
    it('scopes the lookup to the org and 404s otherwise', async () => {
      ...
      expect(prisma.extractionJob.findFirst).toHaveBeenCalledWith({ where: { id: 'job-1', orgId: 'org-1' } });
    });
  });
  ...
  describe('getResult', () => {
    ...
    it('surfaces the stored error message for a FAILED job', async () => {
      const { service, prisma } = makeDeps();
      prisma.extractionJob.findFirst.mockResolvedValue({ id: 'job-1', status: 'FAILED', error: 'Tally unreachable' });
  ...
  describe('getExcelResult', () => {
    it('generates an Items workbook for a STOCK_ITEMS job with no extra params needed', async () => {
      const redisGet = jest.fn().mockResolvedValue(JSON.stringify([{ name: 'Widget', parent: null, description: null, baseUnit: 'Nos', openingBalance: 5, openingValue: 50, closingBalance: 5, closingValue: 50, alterId: 1 }]));
      const { service, prisma, excel } = makeDeps({ redisGet });
      prisma.extractionJob.findFirst.mockResolvedValue({ id: 'job-1', status: 'SUCCESS', type: 'STOCK_ITEMS' });
    ...
    it('rejects a LEDGERS export with no groupsJobId, without touching the excel generator', async () => {
      ...
      prisma.extractionJob.findFirst.mockResolvedValue({ id: 'job-1', status: 'SUCCESS', type: 'LEDGERS' });
    ...
    it('combines a LEDGERS job with a separately-completed GROUPS job to generate the Accounts workbook', async () => {
      ...
      const resultsById: Record<string, unknown> = {
        'ledgers-job': [{ name: 'Aaina Sinha', parent: 'Employee', description: null, openingBalance: 0, closingBalance: 0, alterId: 1 }],
      ...
      expect(excel.generate.mock.calls[0][2]).toEqual([
        expect.objectContaining({ 'Account Name': 'Aaina Sinha', 'Account Type': 'Other Current Liability' }),
      ]);
    });
    ...
    it('rejects a type with no mapper yet (e.g. COMPANIES)', async () => {
      const { service, prisma } = makeDeps({ redisGet: jest.fn().mockResolvedValue(JSON.stringify([])) });
      prisma.extractionJob.findFirst.mockResolvedValue({ id: 'job-1', status: 'SUCCESS', type: 'COMPANIES' });
